                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                @if($techstack->icon)
                                <img src="{{ asset($techstack->icon) }}" alt="{{ $techstack->name }}"
                            ...
                            <td>
                                <div class="d-flex gap-2">
                                    <a href="{{ route('admin.techstacks.show', $techstack->id) }}"
                                    ...
                                    <form action="{{ route('admin.techstacks.destroy', $techstack->id) }}" method="POST"
                                        class="d-inline">
                                        @csrf
                                        @method('DELETE')
            ...
            <section class="sales pt-10 px-7 grid grid-cols-1 md:grid-cols-1 gap-x-7 gap-y-7">
                <div class="flex flex-col gap-y-3">
                    <div class="flex justify-between">
                        ...
                        <a href="{{ route('admin.techstacks.create') }}">
                            <div
                    ...
                    <div class="flex flex-col bg-white rounded-2xl p-5 ">
                        <table class="">
                            <thead class="mb-10">
                                <tr class="flex flex-row items-center grid grid-cols-4 mb-5  justify-between">
                                    <th class="flex flex-row gap-x-3 items-center">
                                        <p class="text-indigo-950 font-semibold text-base">Image</p>
                                    </th>
                                    <th class="flex flex-row gap-x-3 items-center">
                                        <p class="text-indigo-950 font-semibold text-base">Techstacks Name</p>
                                    </th>
                                    <th class="flex flex-row gap-x-3 items-center">
                                        <p class="text-indigo-950 font-semibold text-base">Description</p>
                                    </th>
                                    <th class="flex flex-row gap-x-3 items-center">
                            ...
                            <tbody class="flex-col">
                                @forelse ($techstacks as $techstack)
                                <tr class="flex flex-row items-center mb-5 grid grid-cols-4  justify-between">
                                    ...
                                    <td class="flex flex-row gap-x-3 pr-10 ">
                                        <p class="text-indigo-950 font-medium text-justify text-xs">
                                            {{Str::limit($techstack->description, )}}</p>
                                    </td>
                                    <td class="flex flex-row gap-x-3 items-center">
                                        <a href="{{ route('admin.techstacks.edit', $techstack->id)}}"
                                            class="flex gap-x-2">
                                        ...
                                        <form action="{{ route('admin.techstacks.destroy', $techstack->id) }}"
                                            method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="flex gap-x-2" onclick="return confirm('Are you sure?')">
                                                <div
