        ...
        <div class="flex-auto w-screen lg:pl-[250px]">
            ...
            <section class="sales pt-10 px-7 grid grid-cols-1 md:grid-cols-1 gap-x-7 gap-y-7">
                ...
                <div class="flex-col bg-white rounded-2xl p-5 ">
                    <p class="font-bold">Tambahkan Techstack</p>
                    <div class="h-10"></div>
                    <form action="{{ route('admin.techstacks.store') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3 flex-col flex">
                            <label for="name" class="form-label">Techstacks Name</label>
                            <input type="text" class="bg-gray-50 rounded-lg p-2 border border-gray-200 form-control" id="name" name="name" required>
                            
                        </div>

                        <div class="mb-3 flex-col flex">
                            <label for="icon" class="form-label">Icon</label>
                            <input type="file" class="bg-gray-50 rounded-lg form-control border border-gray-200 p-2" id="icon" name="icon" required >
                            <div class="form-text">Upload an icon image (max 2MB)</div>
                        </div>
                        <div class="mb-3 flex-col flex">
                            <label for="description" class="form-label">Description</label>
                            <textarea class="bg-gray-50 rounded-lg border border-gray-200 p-2" id="description" name="description" rows="3" maxlength="150" required></textarea>
                        </div>
